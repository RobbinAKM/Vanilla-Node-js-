      ...
      checks.forEach(function(check){
        ...
        _data.read('checks',check,function(err,originalCheckData){
          if(!err && originalCheckData){
            ...
            ("Error reading one of the check's data: ",err);
          }
        });
      });
    } else {
...
workers.logs= function(originalCheckData,checkOutcome,state,alertWarranted,timeOfCheck){
...
//rotate logs
 workers.rotateLog=function(){
   //list all the non compress log files in .log folders
   _logs.list(false,(err,logs)=>{
    if(!err && logs && logs.length>0){
      ...
      debug('could not find any logs');
    }
   });
 }

  //call the compression log
